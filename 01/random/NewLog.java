...
public class NewLog {
    ...
    public  static  class TokenizerMapper
        ...
        public void map(Object key, Text value, Context context
           ...
           int count=8000000;//8000000;
           int NumberLog=315;//625;
           int out=0;
           int in=1;
          ...
          for(int i=0;i<count;i++)
          {
              ...
              for(int j=0;j<NumberLog;j++) {
                 ...
                 keyID=new Text(String.valueOf(i));
                  calendar.add(calendar.HOUR,min); //把日期往后增加一天,整数  往后推,负数往前移动
                 ...
                 // System.out.println("int "+intTime);//String.valueOf(calendar.get(Calendar.HOUR_OF_DAY))+":"+String.valueOf(calendar.get(Calendar.MINUTE))+":"+String.valueOf(calendar.get(Calendar.SECOND)); //format.format(date);
                  word=new Text(intTime+" "+String.valueOf(in));
                  context.write(word,keyID);

                  calendar.add(calendar.HOUR,(max-min)); //把日期往后增加一天,整数  往后推,负数往前移动
                  calendar.add(calendar.MINUTE,min2);
                  calendar.add(calendar.SECOND,second2);
                  date=calendar.getTime(); //这个时间就是日期往后推一天的结果
                  String outTime=format.format(date);//String.valueOf(calendar.get(Calendar.HOUR_OF_DAY))+":"+String.valueOf(calendar.get(Calendar.MINUTE))+":"+String.valueOf(calendar.get(Calendar.SECOND));// format.format(date);
